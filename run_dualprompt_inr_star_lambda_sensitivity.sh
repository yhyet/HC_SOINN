#!/bin/bash
# DualPrompt + HC-SOINN + STAR 在 INR 上的 star_lambda 参数敏感性实验脚本
# 依次运行 star_lambda = 0.999, 0.99, 0.95, 0.9 的实验

echo "开始 DualPrompt + HC-SOINN + STAR (INR) 的 star_lambda 参数敏感性实验"
...
for lambda_tag in "${lambda_values[@]}"; do
    config_file="exps/dualprompt_hc_soinn_inr_star_lambda_${lambda_tag}.json"
